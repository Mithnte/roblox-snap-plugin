...
function SelectTool:OnKeyDown(input)
        local settings = self.ctx.settingsStore
        if input.KeyCode == settings:GetKey("switch_select") then
                self.ctx.toolController:SetActive("select")
        elseif input.KeyCode == settings:GetKey("switch_move") then
                self.ctx.toolController:SetActive("move")
        elseif input.KeyCode == settings:GetKey("switch_rotate") then
                self.ctx.toolController:SetActive("rotate")
        elseif input.KeyCode == settings:GetKey("toggle_grid_snap") then
                self.ctx.settingsStore:Set({ gridSnapEnabled = not self.ctx.settingsStore.gridSnapEnabled })
        elseif input.KeyCode == settings:GetKey("toggle_rotate_snap") then
                self.ctx.settingsStore:Set({ rotateSnapEnabled = not self.ctx.settingsStore.rotateSnapEnabled })
        elseif input.KeyCode == settings:GetKey("toggle_vertex_snap") then
                self.ctx.settingsStore:Set({ vertexSnapEnabled = not self.ctx.settingsStore.vertexSnapEnabled })
