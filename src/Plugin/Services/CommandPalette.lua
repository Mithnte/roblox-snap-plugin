...
function CommandPalette:_BuildCommands()
	...
	return {
...

		{
			...
			run = function()
				ctx.settingsStore:Set({ gridSnapEnabled = not ctx.settingsStore.gridSnapEnabled })
			end,
		},
		{
			label = "Toggle Rotate Snap",
			run = function()
				ctx.settingsStore:Set({ rotateSnapEnabled = not ctx.settingsStore.rotateSnapEnabled })
			end,
		},
		{
			label = "Toggle Surface Snap",
			run = function()
				ctx.settingsStore:Set({ surfaceSnapEnabled = not ctx.settingsStore.surfaceSnapEnabled })
			end,
		},
		{
			label = "Toggle Align to Normal",
			run = function()
				ctx.settingsStore:Set({ alignToNormalEnabled = not ctx.settingsStore.alignToNormalEnabled })
			end,
		},

		{
			label = "Align: Center to First",
			run = function()
				ctx.align:CenterToFirst(ctx.selectionStore:Get())
			end,
		},
		{
			label = "Align: Match Rotation to First",
			run = function()
				ctx.align:MatchRotation(ctx.selectionStore:Get())
			end,
		},
		{
			label = "Align: Match Size to First",
			run = function()
				ctx.align:MatchSize(ctx.selectionStore:Get())
			end,
		},

		{
			label = "Array: Linear x5",
			run = function()
				local s = ctx.selectionStore:Get()
				if #s > 0 then
					ctx.array:Linear(s, 5, Vector3.new(ctx.settingsStore.gridStep, 0, 0))
				end
			end,
		},
		{
			label = "Array: Radial x8",
			run = function()
				local s = ctx.selectionStore:Get()
				if #s > 0 then
					ctx.array:Radial(s, 8, 6, 45)
				end
			end,
		},
		{
			label = "Array: Grid 2x2",
			run = function()
				local s = ctx.selectionStore:Get()
				if #s > 0 then
					ctx.array:Grid(s, 2, 2, ctx.settingsStore.gridStep, ctx.settingsStore.gridStep)
				end
			end,
		},

		{
			label = "Paint: Match Appearance to First",
			run = function()
				ctx.paint:MatchAppearanceToFirst(ctx.selectionStore:Get())
			end,
		},
		{
			label = "Paint: Material Neon",
			run = function()
				ctx.paint:SetMaterial(ctx.selectionStore:Get(), "Neon")
			end,
		},
		{
			label = "Paint: Material Wood",
			run = function()
				ctx.paint:SetMaterial(ctx.selectionStore:Get(), "Wood")
			end,
		},
		{
			label = "Paint: Material Metal",
			run = function()
				ctx.paint:SetMaterial(ctx.selectionStore:Get(), "Metal")
			end,
		},
		{
			label = "Paint: Material Plastic",
			run = function()
				ctx.paint:SetMaterial(ctx.selectionStore:Get(), "Plastic")
			end,
		},
		...
		{ label = "Toggle Anchor (Selection)", run = toggleAnchor },
		{
			...
			run = function()
				ctx.settingsStore:ResetToDefaults()
			end,
		},
		{
			label = "Undo",
