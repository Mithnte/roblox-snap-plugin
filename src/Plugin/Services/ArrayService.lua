...
function ArrayService:Linear(selection, n, spacing)
	...
	for i = 1, n do
		...
		-- Translate only this batch by i * spacing so copies land at
		-- 1x, 2x, 3x... spacing instead of compounding on every clone.
		self._ctx.transform:Translate(batch, spacing * i)
	end
	self._ctx.history:Waypoint("Builder: Array Linear (end)")
	return clones
end

-- Radial array: n copies around a shared pivot with radius on XZ and step angle
function ArrayService:Radial(selection, n, radius, stepDeg)
	...
	for i = 1, n do
		...
		local targetCenter = pivot + offset
		for _, inst in ipairs(selection) do
			local c = inst:Clone()
			c.Parent = inst.Parent
			table.insert(clones, c)
			-- Move each clone so it sits around the shared pivot, not around
			-- its own original position, so the array forms an actual circle.
			local basePos = originalPos[inst] or pivot
			local delta = targetCenter - basePos
			self._ctx.transform:Translate({ c }, delta)
		end
	end
...
function ArrayService:Grid(selection, rows, cols, spacingX, spacingZ)
	if #selection == 0 or rows < 1 or cols < 1 then
		return
	end
